...
class EmotionalBehavior:
    def __init__(self, motor_controller=None):
        ...
        # Add tracking for action execution
    ...
    def process_action_queue(self):
        ...
        while self.action_queue:
            ...
            action_type, intensity = self.action_queue.pop(0)
            
            print(f"Processing queued action: {action_type} with intensity {intensity:.1f}")
            self.is_executing_action = True
            self.last_action_time = time.time()
            
            if action_type == "happy":
                self._happy_action(intensity)
            elif action_type == "curious":
                self._curious_action(intensity)
            elif action_type == "surprised":
                self._surprised_action(intensity)
            elif action_type == "seek_attention":
                self._seek_attention_action(intensity)
            elif action_type == "follow_user":
                self._follow_user_action(intensity)
            elif action_type == "find_person":
                self._find_person_action(intensity)
            elif action_type == "approach":
                self.approach_person(intensity)
            elif action_type == "idle":
                self.idle()
            
            # Add a fixed delay between actions to ensure robot completes one before starting another
            time.sleep(0.5)
            
            self.is_executing_action = False
            
        # Reset the thread reference when queue is empty
        self.action_thread = None

    def execute_action(self, action_type, intensity=1.0):
        """Queue an action for execution with given intensity (0.0-1.0)"""
        ...
        if action_type != "follow_user" and current_time - self.last_action_time < 3:
            print(f"Action {action_type} skipped - too soon since last action")
            return False
        
        # Add the action to the queue
        self.action_queue.append((action_type, intensity))
        print(f"Added {action_type} to action queue (queue length: {len(self.action_queue)})")
        
        # Start the processing thread if it's not already running
        if self.action_thread is None:
            self.action_thread = threading.Thread(target=self.process_action_queue)
    ...
    def idle(self):
        ...
        # No action needed for idle, just return
        return True
...
class EmotionDetector:
    def __init__(self):
        ...
        self.emotion_action_cooldowns = {emotion: 0 for emotion in self.emotions}
    ...
    if following_mode:
        ...
        if current_time - last_follow_check > follow_check_interval:
            ...
            if not (hand_results.multi_hand_landmarks and 
                ...
                following_mode = False
                # Make sure to stop motors when exiting following mode
                if behavior.motor_available:
                    behavior.motor.stop()
            last_follow_check = current_time
        ...
        if face_results.detections:
            ...
            # IMPROVED: Control motors directly for smoother continuous movement
            # Define thresholds for centered face
            center_min = 0.4
            center_max = 0.6
            min_distance = 0.2  # Minimum distance to maintain
            
            if behavior.motor_available:
                # Handle turning if person isn't centered
                if face_position_x < center_min:  # Person is to the left
                    # Adjust turn speed based on how far off-center they are
                    turn_intensity = max(0.2, min(1.0, (center_min - face_position_x) * 3))
                    turn_speed = int(30 + turn_intensity * 30)  # 30-60 speed range
                    print(f"Turning left to follow person (intensity: {turn_intensity:.2f})")
                    behavior.motor.left(turn_speed)
                    
                elif face_position_x > center_max:  # Person is to the right
                    # Adjust turn speed based on how far off-center they are
                    turn_intensity = max(0.2, min(1.0, (face_position_x - center_max) * 3))
                    turn_speed = int(30 + turn_intensity * 30)  # 30-60 speed range
                    print(f"Turning right to follow person (intensity: {turn_intensity:.2f})")
                    behavior.motor.right(turn_speed)
                    
                # Move forward if person is centered and not too close
                elif distance_estimate > min_distance:
                    # Adjust forward speed based on distance
                    forward_intensity = min(0.9, max(0.3, distance_estimate * 1.2))
                    forward_speed = int(30 + forward_intensity * 30)  # 30-60 speed range
                    print(f"Moving forward to follow person (speed: {forward_speed})")
                    behavior.motor.forward(forward_speed)
                    
                else:
                    # If we're centered and close enough, just stop
                    print("Stopping - person centered and close enough")
                    behavior.motor.stop()
            else:
                # Simulation logic with appropriate messages
                if face_position_x < center_min:
                    print(f"SIMULATED: Turning left to follow person at position {face_position_x:.2f}")
                elif face_position_x > center_max:
                    print(f"SIMULATED: Turning right to follow person at position {face_position_x:.2f}")
                elif distance_estimate > min_distance:
                    print(f"SIMULATED: Moving forward to follow person at distance {distance_estimate:.2f}")
                else:
                    print("SIMULATED: Stopping - person centered and close enough")
        else:
            # If we lose track of the face but we're still in following mode
            # Stop the motors to prevent runaway
            if behavior.motor_available:
                behavior.motor.stop()
            
            # Every few seconds, try to find the person if they're gone
            if current_time - last_person_search_time > 2.0:  # Search more frequently when following
                print("Lost track of person while following, searching...")
                threading.Thread(target=behavior._find_person_action, 
                              args=(0.5,)).start()
                last_person_search_time = current_time
